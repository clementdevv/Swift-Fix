...
export default function DashboardLayout({ children, userType, navigation }: DashboardLayoutProps) {
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
      {/* ── Top App Bar (Spans Full Width) ── */}
      <header className="bg-white border-b border-gray-200 flex-shrink-0 z-40 relative">
        <div className="px-4 h-16 flex items-center justify-between">
          
          {/* Left section: Toggle + Logo + Page Title */}
          <div className="flex items-center">
            ...
            <div className="flex items-center mr-8">
              <div className="w-10 h-10 bg-[#3B82F6] rounded-full shadow-sm shadow-[#3B82F6]/20 flex items-center justify-center flex-shrink-0">
                <span className="text-white font-bold text-sm tracking-wider">BQ</span>
              </div>
              <div className="ml-3 hidden sm:block">
                <h1 className="text-xl font-bold text-gray-900 leading-tight">Briqoly</h1>
                <p className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold">{userType === 'customer' ? 'Customer' : 'Provider'}</p>
              </div>
            </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {/* Sidebar */}
        <aside className={`
          bg-white border-r border-gray-200 z-30 h-full flex-shrink-0
        ...
        {/* Main Content Area */}
        <main className="flex-1 overflow-auto bg-gray-50 p-6 min-w-0">
          {children}
        </main>
      </div>
